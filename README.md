...
This block was updated in January 2020 to use the new BigQuery logs structure.
...
1. Create a new **Database Connection** in Looker to connect to the BigQuery dataset: follow the steps [here](https://docs.looker.com/setup-and-management/database-config/google-bigquery) to create a service account in GCP and add a new connection to Looker, ensure you use **BigQuery standard SQL**
2. Change the connection name in the model files.
3. In the manifest file, change the connection name in the Looker model files to the connection name you chose in Step 1
4. Change the schema and table names in the manifest file. The table name will likely resemble **cloudaudit_googleapis_com_data_access**.
5. Also change the project name to the current project in the manifest file.
